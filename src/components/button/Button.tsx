...
const Button = ({ xs, sm, md, lg, xl, label, className, border = false, borderRadius = "small", children, color = "violet", colorFont = "white", onClick }: ButtonProps) =>
    <Grid item xs={xs} sm={sm} md={md} lg={lg} xl={xl} className={`p-top-1 p-bottom-1  ${className}`} >
        <button style={{
            width: "100%",
            borderRadius: `var(--border-${borderRadius})`,
            border: border ? `3px solid var(--${colorFont})` : "0px",

        }} className={`background-color-${color} hover`} onClick={onClick}>
            {label ?
